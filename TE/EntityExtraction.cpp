...
vector<int> optimalBeginIndex;
vector<int> optimalEndIndex;
...
int getTE(unsigned int Elength)
{
    if(type == "ED")
        return Elength - q + 1 + tau;
    else if(type == "JS")
...
void initialOptimalBeginIndex(int n)
{
    optimalBeginIndex.clear();
    for(int i = 0;i < n;i++)
        optimalBeginIndex.push_back(0);
}

void initialOptimalEndIndex(int n)
{
    optimalEndIndex.clear();
    for(int i = 0;i < n;i++)
        optimalEndIndex.push_back(0);
}
...
int optimal(int n,int param)
{
    ...
    cout << "TE:" << TE << endl;
//    cout << "n:" << n << endl;
//    cout << "invertList->at(n).second->gette()" << invertList->at(n).second->gette() << endl;
    ...
    for(int i = TE;i < n;i++)
    {
        ...
        for(int j = TE-1;j < i;j++)
        {
            ...
            for(k = j - TE+1;k < j;k++)
            {
                te = invertList->at(k).second->gette();
                if(begin == -1 && k+te>j)
                {
                    begin = k;
                    //cout << "k: " << k << " te: " << te << " j: " << j << endl;
                    //break;
                }
                if(begin != -1 && k+te<=j && listCount[k]>0)
                {
                    erasedList.push_back(k);
                }
            }
            ...
            int erasedSum = 0;
            for(int c = 0;c < erasedList.size();c++)
            {
                erasedSum += listCount[erasedList[c]];
            }

            if(begin>0)
            {
                s = (sum[i] - sum[begin-1] - erasedSum) * (log10(indexTransfer[i].second-indexTransfer[k-1].first+1 - erasedList.size()) / log2 + TE) + indexTransfer[i].second-indexTransfer[begin-1].first+1 - erasedList.size() + dpt[j];
            }
            else
            {
                s = (sum[i] - erasedSum) * (log10(indexTransfer[i].second+1 - erasedList.size()) / log2 + TE) + indexTransfer[i].second+1 - erasedList.size() + dpt[j];
            }

//            s *= log10(i - k) / log2;
//            temp = s + 2 * maxLen * tau + + dpt[j];
            if(s < min)
            {
                cout << "s: " << s << " min: " << min << endl;
                cout << "k: " << begin << " j: " << j << " te: " << te << endl;
                min = s;
                optimalBeginIndex[i]=begin;
                optimalEndIndex[i]=j;
            }
    ...
    beginIndex.clear();
    endIndex.clear();
    endIndex.push_back(optimalEndIndex.size() - 1);
    for(int i = optimalEndIndex[optimalEndIndex.size() - 1];i > 0;)
    {
...
void greedy(int n,double param)
{
    ...
    int TE = getTEAll();
    cout << "TE: " << TE << endl;
    //unsigned long long s = (sum[n] - sum[0]) * log10(n + 1) / log2;
    ...
    for(int i = TE + 1;i <= n;i++)
    {
        ...
        for(j = i - TE;j < i;j++)
        {
            te = invertList->at(j).second->gette();
            if(j+te>i)
            {
                cout << "j: " << j << " te: " << te << " i: " << i << endl;
                break;
            }
        }
        ...
        if(begin>0)
        {
            ts = (sum[i-1] - sum[begin-1]) * (log10(indexTransfer[i-1].second-indexTransfer[begin-1].first+1) / log2 + TE) + indexTransfer[i-1].second-indexTransfer[begin-1].first+1
                + (sum[n-1] - sum[j-1]) * (log10(indexTransfer[n-1].second-indexTransfer[j-1].first+1) / log2 + TE) + indexTransfer[n-1].second-indexTransfer[j-1].first+1;
        }
        else
        {
            ts = sum[i-1] * (log10(indexTransfer[i-1].second+1) / log2 + TE) + indexTransfer[i-1].second+1
                + (sum[n-1] - sum[j-1]) * (log10(indexTransfer[n-1].second-indexTransfer[j-1].first+1) / log2 + TE) + indexTransfer[n-1].second-indexTransfer[j-1].first+1;
...
//        unsigned long long ts = (sum[i-1] - sum[begin]) * (log10(indexTransfer[i-1].second-indexTransfer[begin].second+1) / log2 + TE) + indexTransfer[i-1].second-indexTransfer[begin].second+1 + (sum[n-1] - sum[i - TE]) * (log10(indexTransfer[n-1].second-indexTransfer[i-TE].second+1) / log2 + TE) + indexTransfer[n-1].second-indexTransfer[i-TE].second+1;
//
//        if(ts < s)
//        {
//            cout << "ts: " << ts  << " s: " << s << endl;
...
//                if(j < 0)
//                    j = 0;
//                int tt = invertList->at(j).second->gette();
////                if(tt == 0)
////                {
////                    te = TE;
////                }
...
////            cout << "i: " << i << endl;
//            s = (sum[n-1] - sum[i - te]) * (log10(indexTransfer[n-1].second-indexTransfer[i-te].second+1) / log2 + TE) + indexTransfer[n-1].second-indexTransfer[i-te].second+1;
//        }
    }
    if(begin<n)
    {
...
void random(int n,int param)
{
    ...
    {
        beginIndex.push_back(begin);
        endIndex.push_back(ranindex[i]);
        int j;
        for(j = ranindex[i] - TE;j < ranindex[i];j++)
        {
            if(j < 0)
                j = 0;
            int tt = invertList->at(j).second->gette();
...
void EditDistanceEntry(int argc, char** argv, int limit)
{
  ...
  for(unsigned int i = 0; i < documents.size(); i++)
  {
    ...
    if(algorithm == "Optimal")
    {
        ...
        initialOptimalBeginIndex(documents[i].size() - q + 1);
        initialOptimalEndIndex(documents[i].size() - q + 1);
        initialDpt(documents[i].size() - q + 1);
        optimal(documents[i].size() - q + 1,param);

        for(int i = 0;i < optimalBeginIndex.size();i++)
        {
            cout << optimalBeginIndex[i] << " ";
        }
        cout << endl;
        for(int i = 0;i < optimalEndIndex.size();i++)
        {
            cout << optimalEndIndex[i] << " ";
        }
        cout << endl;
...
FILE *fp = fopen("record_TE_ED.csv","a");
  //fprintf(fp, "\n");
  fprintf(fp, "#%d\n",q );
  fprintf(fp, "#%d\n" , tau );
  fprintf(fp, "#param %.2f\n" , atof(argv[8]) );
...
//        cout << "index.size:" << index->size() << endl;
  ...
  FILE *fp = fopen("record_TE_EDS.csv","a");
  //fprintf(fp, "\n");
  fprintf(fp, "#%d\n",q );
  fprintf(fp, "#%f\n" , det );
  fprintf(fp, "#param %.2f\n" , atof(argv[8]) );
...
//    cout << endl;
    ...
    for(int j = 0;j < beginIndex.size();j++)
    {
        ...
        thisEndIndex = endIndex[j];
        gettimeofday(&bb,0);
//        InvertList* ivl = new InvertList();
//        for(i = 0; i < invertList->size(); i++)
//        {
//            if(invertList->at(i).first != -1)
//            {
//                ivl->push_back(invertList->at(i));
//            }
//        }
//        cout << ivl->size() << endl;
        hbi.jacEntityExtract(invertList,index,documents[i],i,beginIndex[j],endIndex[j]);
        //hbi.jacEntityExtract(ivl,index,documents[i],i);
        gettimeofday(&be,0);
...
//  }
  ...
  FILE *fp = fopen("record_TE_JAC.csv","a");
  //fprintf(fp, "\n");
  fprintf(fp, "#%d\n",q );
  fprintf(fp, "#%f\n" , det );
  fprintf(fp, "#param %.2f\n" , atof(argv[7]) );
